<?php
	...
	{
		function puna_tm_breadcrumbs()
		{
			/* === OPTIONS === */
			$text['home']     = esc_html__( 'Home', 'puna' ); // text for the 'Home' link
			$text['category'] = esc_html__( 'Archive by Category "%s"', 'puna' ); // text for a category page
			$text['tax']      = esc_html__( 'Archive for "%s"', 'puna' ); // text for a taxonomy page
			$text['search']   = esc_html__( 'Search Results for "%s" Query', 'puna' ); // text for a search results page
			$text['tag']      = esc_html__( 'Posts Tagged "%s"', 'puna' ); // text for a tag page
			$text['author']   = esc_html__( 'Articles Posted by %s', 'puna' ); // text for an author page
			$text['404']      = esc_html__( 'Error 404', 'puna' ); // text for the 404 page
			...
			else
			{
				...
				elseif ( is_single() && ! is_attachment() )
				{
					if ( get_post_type() != 'post' )
					{
						...
						if ( $showCurrent == 1 )
						{
							echo esc_html( $delimiter ) . balancetags( $before ) . get_the_title() . balancetags( $after );
						}
					}
					else
					{
						...
						if ( $showCurrent == 1 )
						{
							echo balancetags( $before ) . get_the_title() . balancetags( $after );
						}
					}
				...
				if ( get_query_var( 'paged' ) )
				{
					...
					echo esc_html__( 'Page', 'puna' ) . ' ' . get_query_var( 'paged' );
					if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() )
					{
						echo ')';
					}
